...
def save_images(object_file: str) -> None:
    ...
    camera_dist = np.random.uniform(1.5, 3.0)

    # set the camera elevation
    phi_deg = np.random.uniform(60, 110)
    phi = math.radians(phi_deg)

    # set the object position
    if camera_dist > 2.5:
        obj.location.y += np.random.uniform(-0.5, 0.5)
    if camera_dist <= 2.5 and camera_dist > 2.0:
        obj.location.y += np.random.uniform(-0.3, 0.3)
    elif camera_dist <= 2.0:
        obj.location.y += np.random.uniform(-0.1, 0.1)
